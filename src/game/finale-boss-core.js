...
function armPhase3(next){
  next.chain={active:false,target:null};next.marker=null;next.hazards=[];
  next.formation={row:(next.seed+next.resolved)%3,direction:next.resolved%2===0?1:-1};
}
...
export function resolveVarkosPhase(state,{correct=false,crownCorrect=false}={}){
  ...
  if(next.phase===1){
    ...
    if(correct){next.phase=2;next.phaseIntroSeen=false;armPhase2(next);}
  } else if(next.phase===2){
    if(correct){next.phase=3;next.phaseIntroSeen=false;armPhase3(next);}
  } else if(next.phase===3){
    const {row,direction}=next.formation;
    next.grid=shiftRow(next.grid,row,direction);next.adjacency=adjacency(next.grid);
    if(correct){next.phase=4;next.phaseIntroSeen=false;next.hazards=[];}
...
export function telegraphVarkos(state){
  ...
  if(state.phase===2)return {label:'Phase 2 · Kette und Marker',detail:`Zwei sichtbare Gefahren sind aktiv: ${state.hazards.join(' + ')}. Sprachtext und Primäraktion bleiben frei.`};
  if(state.phase===3)return {label:'Phase 3 · Formation',detail:`Reihe ${state.formation.row+1} wird sichtbar ${state.formation.direction>0?'nach rechts':'nach links'} verschoben; Adjazenz wird danach neu berechnet.`};
