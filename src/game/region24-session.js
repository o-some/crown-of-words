...
export function createRegion24Session(regionId,{errorConcepts=[]}={}){
 ...
 let bossMechanic=regionId==='library'?createBraxState(202):regionId==='wildlife'?createBlackfinnState(303):createRoderickState(errorConcepts);
 return {regionId,phase:'standard',index:0,wordPower:0,solved:0,answers:{},bossIndex:0,bossHp:3,bossMechanic,complete:false};
}
...
export function answerRegion24(session,answer){
 ...
 next.answers[challenge.id]={correct,answer:actual}; if(correct){next.wordPower+=3;next.solved+=1;}
 if(next.phase==='standard') next.index+=1; else {next.bossIndex+=1;if(correct)next.bossHp=Math.max(0,next.bossHp-1); if(next.regionId==='library')next.bossMechanic=resolveBrax(next.bossMechanic,{counterCorrect:Boolean(challenge.counter&&correct)}); if(next.regionId==='wildlife')next.bossMechanic=resolveBlackfinn(next.bossMechanic,{scoutCorrect:Boolean(challenge.counter&&correct)}); if(next.regionId==='home')next.bossMechanic=resolveRoderick(next.bossMechanic,{revengeCorrect:Boolean(challenge.counter&&correct)});}
...
export function startRegion24Boss(session){const next=structuredClone(session);next.phase='boss';next.bossIndex=0;return next;}
